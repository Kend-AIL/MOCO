...
class Corruptdata(Dataset):
    ...
    def normalize_img2kspace(self,img):
        img_2=np.stack((img.real,img.imag),axis=0)
        max_vals=abs(img_2).max(axis=-2, keepdims=True).max(axis=-1, keepdims=True)
        min_vals=abs(img_2).min(axis=-2, keepdims=True).min(axis=-1, keepdims=True)
        img=(img_2-min_vals)/(max_vals-min_vals)
        
        clpx_img=img[0]+img[1]*1j
        kspace=np.fft.fft2(clpx_img)
        out=np.stack((kspace.real,kspace.imag),axis=0)
        return out
    ...
    def __getitem__(self, index):
        ...
        img_raw=self.transform(k_grth,center)
        raw_k_2=self.normalize_img2kspace(img_raw)

# 对每个t维度进行独立的归一化
        
        crop_k_2=self.gen_crop_k_and_target(raw_k_2,noise_mask)
       
        inputs = torch.from_numpy(crop_k_2).to(torch.float32)
        rawk=torch.from_numpy(raw_k_2).to(torch.float32)
...
# 对每个t维度进行独立的归一化
        ...
        mask=torch.from_numpy(noise_mask)

        return inputs,rawk,mask
